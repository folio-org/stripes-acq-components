...
const FundDistributionFields = ({
  fundDistribution, funds, disabled, required, totalAmount, currency, name, stripes,
}) => {
  const distributionTotal = calculateFundDistributionTotal(fundDistribution, totalAmount, currency || stripes.currency);
  const amounts = fundDistribution.reduce((acc, distr, index) => {
    acc[index] = calculateFundAmount(distr, totalAmount, currency || stripes.currency);
