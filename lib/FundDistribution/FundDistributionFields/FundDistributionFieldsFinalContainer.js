...
async function fetchExpenseClasses(fundId, expenseClassesByFundId, setExpenseClassesByFundId, mutator) {
  let classes = null;
  ...
  const onSelectFund = useCallback((fieldName, fundId) => {
    ...
    fetchExpenseClasses(
      ...
      if (classesResponse?.length === 1) {
        change(`${fieldName}.expenseClassId`, classesResponse[0].id);
      }
    });
