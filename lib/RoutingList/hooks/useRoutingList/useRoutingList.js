...
import { LIMIT_MAX, ROUTING_LIST_API, USERS_API } from '../../../constants';
...
export const useRoutingList = (poLineId) => {
  const ky = useOkapiKy();
  const namespace = useNamespace({ key: 'fetch-routing-list' });

  const { isLoading, data } = useQuery({ queryKey: [namespace, poLineId],
    queryFn: async () => {
      const searchParams = {
        limit: LIMIT_MAX,
      ...
      const assignedUserIds = uniq(flatten(routingListRes.map(({ userIds }) => userIds)));
      ...
      return routingListRes.map((routingList) => ({
        ...routingList,
        userIds: routingList.userIds.map((userId) => getUserName(usersMap[userId])),
      }));
    } });
