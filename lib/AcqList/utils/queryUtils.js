...
export const buildDateTimeRangeQuery = (filterKey, filterValue) => {
  const [from, to] = filterValue.split(':');
  // since dates displayed in UTC at FOLIO we filter by that time
  const start = moment.utc(from).startOf('day').format(DATE_RANGE_FILTER_FORMAT);
  const end = moment.utc(to).endOf('day').format(DATE_RANGE_FILTER_FORMAT);
