...
export const buildMultiOptionCqlQuery = (filterKey, filterValue, options = {}) => {
  ...
  const checkIsEmptyArrayValue = (value) => {
    if (value instanceof CQLBuilderValue) {
      return Object.is(new CQLBuilderStringValue(emptyArrayFilterValue).toCQL(), value.toCQL());
    }

    return value === emptyArrayFilterValue;
  };

  const value = Array.isArray(filterValue) ? new CQLBuilderArrayValue(filterValue) : filterValue;
  const isEmptyArrayValue = checkIsEmptyArrayValue(value);
