...
export const buildDateTimeRangeQuery = (filterArgs, filterValue) => {
  const [filterKey, timezone = TIMEZONE] = filterArgs;
  const [from, to] = filterValue.split(':');

  const start = moment.tz(from, timezone).startOf('day').format();
  const end = moment.tz(to, timezone).endOf('day').format();

  const startUTC = moment.tz(start, TIMEZONE).format();
  const endUTC = moment.tz(end, TIMEZONE).format();

  return `(${filterKey}>="${startUTC}" and ${filterKey}<="${endUTC}")`;
};
