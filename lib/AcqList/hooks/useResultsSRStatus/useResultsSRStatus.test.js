...
import { useResultsSRStatus } from './useResultsSRStatus';
...
describe('useResultsSRStatus', () => {
  ...
  it('should send SR status message when isLoading = true', async () => {
    const readerStatusMock = {
      sendMessage: jest.fn(),
    };
    const { rerender, result } = renderHook((props = {}) => useResultsSRStatus(props));
    ...
    rerender({ isLoading: true, hasFilters: true, resultsCount: 5 });

    expect(readerStatusMock.sendMessage).not.toHaveBeenCalled();
  });
});
