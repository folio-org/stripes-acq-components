...
const useLocationFilters = (
  ...
  const applyLocationFilters = useCallback(
    (type, value, updateURL = true, keepURLParameters = true) => {
      const newFilters = applyFilters(type, value);

      if (updateURL) {
        history.push({
          pathname: '',
          search: `${buildSearch(newFilters, keepURLParameters ? location.search : null)}`,
        });
      }
  ...
  const applyLocationFiltersAsync = useCallback(
    async (type, value, updateURL = true) => {
      const newFilters = await applyFiltersAsync(type, value);
    ...
    [applyFiltersAsync, history, location.search],
  );

  const applyLocationsSearch = useCallback(
    () => applyLocationFilters(SEARCH_PARAMETER, searchQuery, true, false),
