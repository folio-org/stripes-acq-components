...
export const FieldHolding = ({
  ...
  useEffect(() => {
    setIsHoldingIdValid(!values?.holdingId || holdings.some(({ id }) => id === values.holdingId));
  }, [holdings]);

  useEffect(() => {
    if (!values.locationId) setSelectedLocation();
  }, [values.locationId]);

  const holdingOptions = useMemo(() => (
    getHoldingOptions(isHoldingIdValid ? holdings : [...holdings, { id: values.holdingId }], locationsMap)
  ), [holdings, locationsMap, isHoldingIdValid]);
