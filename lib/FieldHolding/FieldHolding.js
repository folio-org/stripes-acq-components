...
export const FieldHolding = ({
  ...
  const initialInstanceId = initialValues?.instanceId;
  ...
  /**
   * Memoized holding options for the select dropdown.
   *
   * If the holding ID is invalid and we're viewing the initial instance,
   * adds the initial holding to the list of available holdings before filtering.
   * This ensures the initial holding remains selectable even if it's not in the
   * main holdings collection.
   *
   * Recalculates whenever the instance context, holding validity, or available
   * holdings change.
   *
   * @type {Array<Object>} Array of holding option objects with id and label properties
   */
  const holdingOptions = useMemo(() => {
    const isInitialInstance = instanceId === initialInstanceId;

    const holdingsToSelect = !isHoldingIdValid && isInitialInstance
      ? [...holdings, { id: initialHoldingId }]
      : holdings;
