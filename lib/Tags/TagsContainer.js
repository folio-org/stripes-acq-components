...
const TagsContainer = (props) => {
  ...
  const saveEntityTags = useCallback(
    (tags) => {
      ...
      putMutator(updatedRecordObj)
        .catch(() => {
          showCallout(<FormattedMessage id="stripes-smart-components.cannotSaveTagToRecord" />, 'error');
        });
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [recordObj, showCallout],
  );
  ...
  const saveTags = useCallback(
    (tags) => {
      ...
      mutator.tags.POST({
        ...
        .catch(() => showCallout(<FormattedMessage id="stripes-smart-components.cannotSaveNewTag" />, 'error'));
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [allTags, showCallout],
  );
