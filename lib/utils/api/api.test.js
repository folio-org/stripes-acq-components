...
import { fetchFundExpenseClasses } from './fetchFundExpenseClasses';
import { fetchFunds } from './fetchFunds';
import { fetchHoldingsRelatedEntities } from './fetchHoldingsRelatedEntities';
...
import { fetchTenantAddressById } from './fetchTenantAddressById';
import { fetchTenantAddresses } from './fetchTenantAddresses';
import { fetchTenantAddressesByIds } from './fetchTenantAddressesByIds';
...
describe('API utils', () => {
  ...
  describe('fetchFundExpenseClasses', () => {
    it('should fetch expense classes for a specific fund', async () => {
      const fundId = 'fund-uuid';

      await fetchFundExpenseClasses(httpClient)(fundId, options);

      expect(httpClient.get).toHaveBeenCalledWith(`${FUNDS_API}/${fundId}/expense-classes`, options);
    });

    it('should pass options through to the http client', async () => {
      const fundId = 'fund-uuid';
      const searchOptions = { searchParams: { status: 'Active', fiscalYearId: 'fy1' } };

      await fetchFundExpenseClasses(httpClient)(fundId, searchOptions);

      expect(httpClient.get).toHaveBeenCalledWith(`${FUNDS_API}/${fundId}/expense-classes`, searchOptions);
    });
  });
  ...
  describe('fetchHoldingsRelatedEntities', () => {
    it('should retrieve relations details for holdings by IDs', async () => {
      await fetchHoldingsRelatedEntities(httpClient)(ids, options);
  ...
  it('should fetch tenant address by id', async () => {
    const addressId = 'addressId';

    await fetchTenantAddressById(httpClient)(addressId, options);

    expect(httpClient.get).toHaveBeenCalledWith(`${TENANT_ADDRESSES_API}/${addressId}`, options);
  });

  it('should fetch tenant addresses', async () => {
    await fetchTenantAddresses(httpClient)(options);

    expect(httpClient.get).toHaveBeenCalledWith(TENANT_ADDRESSES_API, options);
  });

  it('should fetch tenant addresses by ids', async () => {
    await fetchTenantAddressesByIds(httpClient)(ids, options);

    expect(httpClient.get).toHaveBeenCalledWith(TENANT_ADDRESSES_API, { searchParams });
  });
