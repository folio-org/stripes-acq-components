...
export const getInvoiceExportData = async ({ ky, intl, query, currency: to }) => {
  ...
  const [
    ...
    fetchExportDataByIds({ ky, ids: acqUnitsIds, api: ACQUISITIONS_UNITS_API, records: 'acquisitionsUnits' }),
    fetchTenantAddressesByIds(ky)(addressIds).then(({ addresses: tenantAddresses }) => tenantAddresses),
    fetchExportDataByIds({ ky, ids: batchGroupIds, api: BATCH_GROUPS_API, records: 'batchGroups' }),
    fetchExportDataByIds({ ky, ids: expenseClassIds, api: EXPENSE_CLASSES_API, records: 'expenseClasses' }),
    fetchExportDataByIds({ ky, ids: poLineIds, api: LINES_API, records: 'poLines' }),
    fetchExportDataByIds({ ky, ids: userIds, api: USERS_API, records: 'users' }),
    fetchExportDataByIds({ ky, ids: vendorIds, api: VENDORS_API, records: 'organizations' }),
