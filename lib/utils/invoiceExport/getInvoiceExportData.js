...
export const getInvoiceExportData = async ({ ky, intl, query, currency: to }) => {
  ...
  const buildAddressQuery = (itemsChunk) => {
    ...
    return subQuery ? `(scope=${SETTINGS_SCOPES.TENANT_ADDRESSES} and (${subQuery}))` : '';
  };
  const addressRecords = await fetchExportDataByIds({
    ky, ids: addressIds, buildQuery: buildAddressQuery, api: SETTINGS_ENTRIES_API, records: 'items',
  });
