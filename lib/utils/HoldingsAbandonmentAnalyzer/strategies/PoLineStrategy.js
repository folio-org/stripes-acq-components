...
export class HoldingsAbandonmentPOLineStrategy extends HoldingsAbandonmentBaseStrategy {
  ...
  static analyzeHoldings({
    ...
    return holdingIds.map((holdingId) => {
      ...
      let tenantId;
      const pieceWithTenant = pieces.find((p) => p.tenantId);

      if (pieceWithTenant) {
        tenantId = pieceWithTenant.tenantId;
      } else {
        const itemWithTenant = items.find((i) => i.tenantId);

        if (itemWithTenant) {
          tenantId = itemWithTenant.tenantId;
        }
      }
