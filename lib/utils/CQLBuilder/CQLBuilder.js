...
export class CQLBuilder {
   ...
   */
  exact(field, value) {
    return this._addCondition(field, '==', value);
  }
  ...
  contains(field, value) {
    return this._addCondition(field, 'all', value);
  }
  ...
  containsAny(field, value) {
    return this._addCondition(field, 'any', value);
  }

  allRecords() {
    return this.fuzzy('cql.allRecords', 1);
  }
  ...
  orModifiersSearch(field, modifiers, searchValue) {
    ...
    const modifiersChain = modifiersArray.map(m => `/@${m}`).join('');
  ...
  sortByMultiple(sorts) {
    ...
    this.queryParts.push(`sortBy ${sortClauses.join(' ')}`);
  ...
  _lastPartIsOperator() {
    ...
    return ['AND', 'OR', 'NOT', '('].includes(last);
  }
