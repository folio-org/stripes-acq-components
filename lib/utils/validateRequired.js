...
export function validateRequiredMinAndMaxNumber({ minNumber, maxNumber, value }) {
  ...
  if (value > maxNumber) {
    return <FormattedMessage id="ui-organizations.validation.shouldBeLessThan" values={{ maxNumber }} />;
  }

  if (value < minNumber) {
    return <FormattedMessage id="ui-organizations.validation.shouldBeGreaterThan" values={{ minNumber }} />;
  }
