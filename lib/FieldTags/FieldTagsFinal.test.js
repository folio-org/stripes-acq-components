...
describe('FieldTagsFinal', () => {
  ...
  it('should display preselected tags and handle clicks', async () => {
    ...
    await user.click(screen.getAllByLabelText('times')[0]);
  ...
  it('should add newly created tag to the selected list', async () => {
    ...
    await user.type(screen.getByRole('combobox', { name: 'stripes-acq-components.label.tags' }), 'qwerty');
    await user.click(screen.getByText('stripes-acq-components.addTagFor'));
